...
class Node {
  ...
  public:
    ...
    inline bool operator==(const NodePtr &rhs) { return index_ == rhs->index_; }

    inline unsigned int getIndex() const { return index_; }

    inline float getCost() const { return cell_cost_; }

    inline void setCost(const float &cost) { cell_cost_ = cost; }

    inline bool wasVisited() { return type_ == LISTYPE::CLOSELIST; }

    inline void visited() { type_ = LISTYPE::CLOSELIST; }

    inline void queued() { type_ = LISTYPE::OPENLIST; }

    inline bool wasQueued() { return type_ == LISTYPE::OPENLIST; }

    inline bool wasUnknown() { return type_ == LISTYPE::UNKNOW; }

    inline Eigen::Vector2i getCoord() { return coord_; }

    inline float getCoordX() { return coord_(0); }

    inline float getCoordY() { return coord_(1); }
