...
#include "../include/nav2_astarline_planner/Astar.hpp"
...
//       assert(neighbors_grid_offsets_.size() ==
//       neighbors_traversal_cost_.size()); break;
//     case MotionModel::MOORE:
//       neighbors_grid_offsets_ = {-1, +1, -xs_, +xs_, -xs_ - 1,
//         -xs_ + 1, +xs_ - 1, +xs_ + 1};
//       neighbors_traversal_cost_ = {1.0, 1.0, 1.0, 1.0, sqrt2, sqrt2, sqrt2,
//       sqrt2}; assert(neighbors_grid_offsets_.size() ==
//       neighbors_traversal_cost_.size()); break;
//     default:
...
bool Astar::createPath(const Eigen::Vector2i &start, const Eigen::Vector2i &end,
    ...
    if (start_i < 0 || start_i > ns_ || end_i < 0 || end_i > ns_) {
        std::cerr << "Astar: start or end index illegal " << start << " " << end
                  << std::endl;
    }
    ...
    while (!open_list_.empty()) {
        ...
        expansiionNeighbors(current->getIndex(), neighbors_index,
                            neighbors_cost);
        ...
        for (int i = 0; i < neighbors_index.size(); ++i) {
            ...
            if (t_i < ns_ && t_i >= 0) {
                ...
                if (tmp->getCost() >= OCCUPIED || tmp->wasVisited()) {
                    continue;
                }
    ...
    if (find_path) {
        ...
        if (!backtracePath(end_ptr, path_reverse)) {
            std::cout << "FATAL ERROR, find path but backtrace path field"
                      << std::endl;
            return false;
        } else {
            path.clear();
...
void Astar::expansiionNeighbors(const int &current_index,
                                std::vector<int> &neighbors_index,
                                std::vector<float> &neighbors_cost) {
    ...
    if (current_index + xs_ + 1 < ns_ && (current_index + xs_ + 1) % xs_ != 0) {
        neighbors_index.push_back(current_index + xs_ + 1); // right_down
        neighbors_cost.push_back(sqrt2);
    }

    if (current_index - xs_ + 1 >= 0 && (current_index - xs_ + 1) % xs_ != 0) {
